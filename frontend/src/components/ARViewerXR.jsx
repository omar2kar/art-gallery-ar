...
const VERSION = 'AR v4'; // علامة إصدار — للتأكد من تحميل آخر نسخة (ليست cache)
  ...
  const startAR = useCallback(async () => {
    try {
      ...
      const session = await navigator.xr.requestSession('immersive-ar', {
        ...
        optionalFeatures: [
          'anchors', 'light-estimation', 'dom-overlay',
      ...
      for (const type of ['local-floor', 'local', 'unbounded', 'viewer']) {
        try { localSpace = await session.requestReferenceSpace(type); worldType = type; break; }
        catch { /* جرّب النوع التالي */ }
      }
      if (!localSpace) throw new Error('الجهاز لا يدعم أي نوع فضاء مرجعي معروف');
    ...
    <div style={dark}>
      ...
      {support === 'ready' && !running && (
        <div style={{ textAlign: 'center', padding: '2rem', maxWidth: 360, width: '100%' }}>
          ...
          {diag && (
            <p style={{ color: '#e8c45a', fontSize: '0.75rem', marginBottom: '1rem',
                        background: 'rgba(232,196,90,0.08)', padding: '0.5rem 0.7rem',
                        borderRadius: 8, lineHeight: 1.5 }}>{diag}</p>
          )}
          ...
          <button onClick={() => setForceOld(true)}
            style={{ width: '100%', background: 'rgba(255,255,255,0.07)',
                     border: '1px solid rgba(255,255,255,0.15)', color: 'white',
                     padding: '0.8rem', borderRadius: 12, cursor: 'pointer',
                     fontSize: '0.9rem', marginBottom: '0.9rem' }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.25)', fontSize: '0.7rem', marginTop: '1rem' }}>
            {VERSION}
          </p>
        </div>
      )}
